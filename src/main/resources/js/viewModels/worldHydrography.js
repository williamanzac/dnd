define(
		[ 'jquery', 'knockout', 'Modal', '../components/worldHydrography',
				'../components/displayType',
				'../components/landWaterDistribution',
				'../components/landWaterMass', '../components/regionType',
				'knockstrap' ],
		function($, ko, Modal, WorldHydrography, DisplayType,
				LandWaterDistribution, LandWaterMass, RegionType) {
			return function HydrographyViewModel() {
				...
				self.editLandWaterDistribution = ko
						.observable(new LandWaterDistribution({
							displayType : null,
							regionType : null,
							hydrography : null
						}));
				...
				self.editLandWaterDistributionModal = new Modal(
						self.editLandWaterDistribution);
				...
				self.newLandWaterDistributionModal = new Modal(
						self.editLandWaterDistribution);
				self.newLandWaterMassModal = new Modal(self.editLandWaterMass);

				self.showNewLandWaterDistributionModal = function(regionType,
						displayType, hydrography) {
				...
				self.generateHydrography = function() {
					...
					self.results().forEach(
							function(d) {
								LandWaterMass.generate(self.massResults, d.id,
										self.selectedDisplayType().id);
							});
				}

				self.findNumRegions = function(regionType, displayType,
						hydrography) {
